function min_d = min_dist_between_two_polygons(Poly1,Poly2,weightx, weighty,Display_solution)
P1.x=Poly1(:,1);
P1.y=Poly1(:,2);
P2.x=Poly2(:,1);
P2.y=Poly2(:,2);
...
elseif Npts_P1==1 && Npts_P2==1
    min_d = sqrt((weightx^2)*(P1.x-P2.x)^2+(weighty^2)*(P1.y-P2.y)^2);
    return;
end
...
if Npts_P1 >= 3
     dP1x = diff(P1.x);
     dP1y = diff(P1.y);
    sdP12 = (weightx^2)*dP1x.^2 + (weighty^2)*dP1y.^2;
    if any(sdP12 == 0)
    ...
    for i = 1:numel(P1.x)-1
        ...
        k = -((weightx^2)*dP1x(i) * (P1.x(i) - P2.x) +...
              (weighty^2)*dP1y(i) * (P1.y(i) - P2.y)) / sdP12(i);
        I1 = k < 0;    
        I2 = k > 1;
        I  =~(I1|I2);
        % Computation of the minimum squared distances between all points of 
        % P2 and the i-th segment of P1.
        D1(i,I1) = (weightx^2)*((P2.x(I1)- P1.x(i  )).^2)+(weighty^2)*((P2.y(I1)-P1.y(i  )).^2);
        D1(i,I2) = (weightx^2)*((P2.x(I2)- P1.x(i+1)).^2)+(weighty^2)*((P2.y(I2)-P1.y(i+1)).^2);
        D1(i,I ) = (weightx^2)*((P2.x(I )-(P1.x(i)+k(I)*dP1x(i))).^2)+...
                       (weighty^2)*((P2.y(I )-(P1.y(i)+k(I)*dP1y(i))).^2);
        K1(i,:) = k;
    end
...
if Npts_P2 >= 3
     dP2x = diff(P2.x);
     dP2y = diff(P2.y);
    sdP22 = (weightx^2)*dP2x.^2 + (weighty^2)*dP2y.^2;
    if any(sdP22 == 0)
    ...
    for i = 1:Npts_P2-1
        k = -((weightx^2)*dP2x(i) * (P2.x(i) - P1.x) + ...
              (weighty^2)*dP2y(i) * (P2.y(i) - P1.y))/sdP22(i);
        I1 = k < 0;
        I2 = k > 1;
        I  =~(I1|I2);
        D2(i,I1) = (weightx^2)*((P1.x(I1)-P2.x(i  )).^2)+(weighty^2)*((P1.y(I1)-P2.y(i  )).^2);
        D2(i,I2) = (weightx^2)*((P1.x(I2)-P2.x(i+1)).^2)+(weighty^2)*((P1.y(I2)-P2.y(i+1)).^2);
        D2(i,I ) = (weightx^2)*((P1.x(I )-(P2.x(i)+k(I)*dP2x(i))).^2)+...
                        (weighty^2)*((P1.y(I )-(P2.y(i)+k(I)*dP2y(i))).^2);
        K2(i,:) = k;
    end
