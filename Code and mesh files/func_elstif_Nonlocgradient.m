function [k_el, residual_el, kappa_el, gausspoints_prop_mat_elem, nodes_prop_mat_elem] = func_elstif_Nonlocgradient(lmncoord,dofs,Delastic,kappa_el_previousinc,g,alpha_val,beta_val,e_delta,dmax,n_hood,weights,IsM,IsProj)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if IsProj == 0
    ...
    for integ_point = 1:npoints
        ...
        % ======================== STIFFNESS MATRICES =========================
        k_uu_el = k_uu_el + (1 - damage_el(1,integ_point)) * Bu' * Delastic * Bu * w(integ_point) * dt;   % Size: 8x8 (1x1 * 8x3 * 3x3 * 3x8 * 1x1 * 1x1)
        k_ue_el = k_ue_el + Bu' * Delastic * strain_vec_gxy * N' * domega_dkappa * w(integ_point) * dt;   % Size: 8x4 (8x3 * 3x3 * 3x1 * 1x4 * 1x1 * 1x1 * 1x1)
        k_eu_el = k_eu_el + N * s' * Bu * w(integ_point) * dt;                                            % Size: 4x8 (4x1 * 1x3 * 3x8 * 1x1 * 1x1)
        k_ee_el = k_ee_el + (N * N' + g * (Be' * Be)) * w(integ_point) * dt;                              % Size: 4x4 (4x1 * 1x4 + 1x1 * 4x2 * 2x4 * 1x1 * 1x1)
    ...
    k_el = [k_uu_el -k_ue_el; -k_eu_el k_ee_el];      % Size: 12x12
    
    
    % =============== ELEMENT INTERNAL FORCE VECTOR continued =================
    % Assemble the element internal force 
    f_internal_ee_el = k_ee_el * dofs_e_vec;              % Size: 4x1 (4x4 * 4x1)
